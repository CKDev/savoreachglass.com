...
Element.getOpacity = function(element) {
  ...
  if (
    (opacity = (element.getStyle("filter") || "").match(
      /alpha\(opacity=(.*)\)/
    ))
  )
...
Element.setOpacity = function(element, value) {
  ...
  if (value == 1) {
    ...
    if (/MSIE/.test(navigator.userAgent) && !window.opera)
      element.setStyle({
        filter: Element.getStyle(element, "filter").replace(
          /alpha\([^\)]*\)/gi,
          ""
        )
      });
...
Object.extend(Object.extend(Effect.ScopedQueue.prototype, Enumerable), {
  ...
  add: function(effect) {
    ...
    var position =
      typeof effect.options.queue == "string"
        ...
        backgroundColor: $R(0, 2).inject(
          ...
          function(m, v, i) {
            return (
              m +
              Math.round(
                this._base[i] + this._delta[i] * position
              ).toColorPart()
            );
          }.bind(this)
        )
...
Object.extend(Object.extend(Effect.Morph.prototype, Effect.Base.prototype), {
  initialize: function(element) {
    ...
    if (typeof options.style == "string") {
      if (options.style.charAt(0) == ".") {
        var cssText = "";
        $A(document.styleSheets)
          .reverse()
          .each(function(styleSheet) {
            if (styleSheet.cssRules) cssRules = styleSheet.cssRules;
            else if (styleSheet.rules) cssRules = styleSheet.rules;
            $A(cssRules)
              .reverse()
              .each(function(rule) {
                if (options.style == rule.selectorText) {
        ...
        options.afterFinishInternal = function(effect) {
          effect.element.addClassName(effect.options.style.substring(1));
  ...
  setup: function() {
    ...
    this.transforms = this.style
      .map(
        function(pair) {
          var property = pair[0].underscore().dasherize(),
            value = pair[1],
            unit = null;
        ...
        transform.unit == "color"
          ? $R(0, 2).inject("#", function(m, v, i) {
              return (
                m +
                Math.round(
                  transform.originalValue[i] +
                    (transform.targetValue[i] - transform.originalValue[i]) *
                      position
                ).toColorPart()
              );
            })
