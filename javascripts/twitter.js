  ...
  (function() {
    ...
    window.renderTwitters = function(obj, options) {
      ...
      for (i = 0; i < max && obj[i]; i++) {
        ...
        if (options.template) {
          li.innerHTML = options.template.replace(/%([a-z_\-\.]*)%/gi, function(
            m,
            l
          ) {
            var r = data[l] + "" || "";
            ...
            if (options.timeout) {
              window["twitterTimeout" + guid] = setTimeout(function() {
                ...
                options.onTimeout.call(
                  document.getElementById(options.twitterTarget)
                );
    ...
    function relative_time(time_value) {
      ...
      if (delta < 5) {
        ...
        // 2* because sometimes read 1 hours ago
        r = "about 1 hour ago";
